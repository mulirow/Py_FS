...
def RDA_Mod_3(num_agents, max_iter, train_data, train_label, obj_function=compute_fitness, trans_function_shape='s', save_conv_graph=False, weight_acc=0.9, val_size=0.2):
    ...
    for iter_no in range(max_iter):
        ...
        for i in range(num_coms):
            ...
            r1 = np.random.random()
            r2 = np.random.random()
            new_male_1 = (chosen_com + chosen_stag) / 2 + r1 * (((UB - LB) * r2)) # Eq. (6)
            new_male_2 = (chosen_com + chosen_stag) / 2 - r1 * (((UB - LB) * r2)) # Eq. (7)
        ...
        for i in range(num_coms):
            ...
            for j in range(num_harem_mate[i]):
                r1, r2 = np.random.rand(2) # r is a random number in [0, 1]
                avg = (coms[i] + harem[i][j]) / 2
                if r2 >= 0.5:
                    offspring = avg + (UB - avg) * r1 # Eq. (12)
                else:
                    offspring = avg - (avg - LB) * r1
...
if __name__ == '__main__':
    ...
    RDA_Mod_3(20, 100, data.data, data.target, save_conv_graph=True)
############# for testing purpose ################
