...
def RDA_Mod_4(num_agents, max_iter, train_data, train_label, obj_function=compute_fitness, trans_function_shape='s', save_conv_graph=False, weight_acc=0.9, val_size=0.2):
    ...
    for iter_no in range(max_iter):
        ...
        for i in range(num_males):
            r1, r2, r3 = np.random.rand(3, num_features) # r_i is an array with j numbers in [0, 1]
            new_male = males[i].copy()

            for j in range(num_features):
                if r3[j] >= 0.5:
                    new_male[j] += r1[j] * (((UB - LB) * r2[j])) # Eq. (3)
                else:
                    new_male[j] -= r1[j] * (((UB - LB) * r2[j]))
        ...
        for i in range(num_coms):
            ...
            r1, r2 = np.random.rand(2, num_features)
            avg = (chosen_com + chosen_stag) / 2
            new_male_1 = avg + r1 * (((UB - LB) * r2)) # Eq. (6)
            new_male_2 = avg - r1 * (((UB - LB) * r2)) # Eq. (7)
        ...
        for i in range(num_coms):
            ...
            for j in range(num_harem_mate[i]):
                ...
                offspring = np.zeros(num_features)
                for k in range(num_features):
                    if r1[k] >= 0.5:
                        offspring[k] = avg[k] + (UB - avg[k]) * r2[k] # Eq. (12)
                    else:
                        offspring[k] = avg[k] - (avg[k] - LB) * r2[k]

                # apply transformation function on offspring
                for k in range(num_features):
                    trans_value = trans_function(offspring[k])
                    if (np.random.random() < trans_value):
                        offspring[k] = 1
                    else:
                        offspring[k] = 0
                ...
                if num_coms > 1:
                    ...
                    for l in range(num_mate):
                        r1, r2 = np.random.rand(2, num_features) # r is a random number in [0, 1]
                        avg = (coms[i] + harem[k][l]) / 2
                        offspring = np.zeros(num_features)
                        for k in range(num_features):
                            if r1[k] >= 0.5:
                                offspring[k] = avg[k] + (UB - avg[k]) * r2[k] # Eq. (12)
                        ...
                        for m in range(num_features):
                            trans_value = trans_function(offspring[m])
                            if (np.random.random() < trans_value):
                                offspring[m] = 1
                            else:
                                offspring[m] = 0
...
if __name__ == '__main__':
    ...
    RDA_Mod_4(20, 100, data.data, data.target, save_conv_graph=True)
############# for testing purpose ################
